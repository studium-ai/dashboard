...
merged_df= data.table::rbindlist(list(df1, df2, df3, df4), use.names = FALSE)

merged_total_persons = nrow(ps_att |> distinct(ps_id, .keep_all = TRUE) |> 
                           filter(str_detect(import, "Matrikel")))

merged_ps_att = ps_att |> distinct(ps_att_id, .keep_all = TRUE) |> 
  filter(str_detect(import, "Matrikel")) |> pull(ps_att_id)

merged_total_institutions = nrow(ps_att_institutions |> filter(ps_att_id %in% merged_ps_att) |> distinct(institution_id))

merged_total_sources = nrow(ps_att |> distinct(source_id, .keep_all = TRUE) |> 
                           filter(str_detect(import, "Matrikel")))

merged_total_places = nrow(descriptors |> filter(V5 %in% mat_ps_att) |> distinct(V8))
