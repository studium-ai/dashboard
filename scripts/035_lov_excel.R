...
sections <- list(
  "Roles" = lov_role,
  "Wealth Category" = lov_wealth,
  "Pedagogies" = lov_institution,
  "Institutions" = lov_source_inst
)
...
for (i in seq_along(sections)) {
  ...
  df <- sections[[i]]
  current_row <- write_section(title, df, wb, "Lovaniensia", current_row)
}
